...
@Component({
  ...
  styleUrls: ['./knowledge-criteria.component.css']
})
export class KnowledgeCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'knowledge' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('knowledge/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
