...
@Component({
  selector: 'app-poll-category',
  templateUrl: './poll-category.component.html',
  styleUrls: ['./poll-category.component.css']
})
export class PollCategoryComponent implements OnInit {
  model: any = [];
  listModel: string = '';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('pollCategoryPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('pollCategoryPage'));
  ...
  read() {
    ...
    this.serviceProviderService.post('poll/category/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
