...
@Component({
  selector: 'app-welfare-criteria',
  templateUrl: './welfare-criteria.component.html',
  styleUrls: ['./welfare-criteria.component.css']
})
export class WelfareCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'welfare' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('welfare/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
