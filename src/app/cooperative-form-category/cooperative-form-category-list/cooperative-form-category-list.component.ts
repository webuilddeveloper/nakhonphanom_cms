...
@Component({
  selector: 'app-cooperative-form-category-list',
  templateUrl: './cooperative-form-category-list.component.html',
  styleUrls: ['./cooperative-form-category-list.component.css']
})
export class CooperativeFormCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('cooperativeFormCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('cooperativeFormCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['cooperative-form-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['cooperative-form-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('cooperativeForm/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('cooperativeForm/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
