...
export class ContactListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('contactPage') != null) {
      let model: any = [];
      this.permission = JSON.parse(localStorage.getItem('contactPage'));
  ...
  create() {
    this.router.navigate(['contact-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['contact-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('contact/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('contact/reorder', { code: model }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('contact/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          },2000)
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('contact/read', {'skip':0,'limit':999999,'permission':this.permissionStr}).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
      ...
      this.excelService.exportAsExcelFile(result, 'รายงานเบอร์โทรฉุกเฉิน');
    }, err => {
      this.toastr.error(err.message, 'แจ้งเตือนระบบ', { timeOut: 2000 });
    });
  }
