...
export class SuggestionListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('suggestionPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('suggestionPage'));
    }
  ...
  create() {
    this.router.navigate(['suggestion-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['suggestion-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('suggestion/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('suggestion/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
