...
@Component({
  selector: 'app-privilege-criteria',
  templateUrl: './privilege-criteria.component.html',
  styleUrls: ['./privilege-criteria.component.css']
})
export class PrivilegeCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'privilege' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('privilege/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
