...
@Component({
  selector: 'app-cooperative-form-list',
  templateUrl: './cooperative-form-list.component.html',
  styleUrls: ['./cooperative-form-list.component.css']
})
export class CooperativeFormListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('cooperativeFormPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('cooperativeFormPage'));
    }
  ...
  create() {
    this.router.navigate(['cooperative-form-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['cooperative-form-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('cooperativeForm/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('privilege/reorder', { code: model }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('cooperativeForm/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          },2000)
