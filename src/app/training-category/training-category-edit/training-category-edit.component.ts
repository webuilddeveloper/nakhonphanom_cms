...
@Component({
  selector: 'app-training-category-edit',
  templateUrl: './training-category-edit.component.html',
  styleUrls: ['./training-category-edit.component.css']
})
export class TrainingCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่ฝึกอบรม';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่ฝึกอบรม';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('training/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('training/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('training/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['training-category'], { skipLocationChange: true });
  }
