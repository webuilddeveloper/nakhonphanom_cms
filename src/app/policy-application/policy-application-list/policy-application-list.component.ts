...
@Component({
  ...
  templateUrl: './policy-application-list.component.html',
  styleUrls: ['./policy-application-list.component.css']
})
export class PolicyApplicationListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('policyApplicationPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('policyApplicationPage'));
    }
  ...
  create() {
    this.router.navigate(['policy-application-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['policy-application-edit', param], { skipLocationChange: true });
  }
