...
@Component({
  selector: 'app-policy-application-criteria',
  templateUrl: './policy-application-criteria.component.html',
  styleUrls: ['./policy-application-criteria.component.css']
})
export class PolicyApplicationCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('policyApplicationPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('policyApplicationPage'));
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'policy' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  reset() {
    ...
    this.criteriaModel.category = 'application';
