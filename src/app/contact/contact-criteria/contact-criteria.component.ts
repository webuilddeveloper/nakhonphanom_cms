...
export class ContactCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'contact' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('contact/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
