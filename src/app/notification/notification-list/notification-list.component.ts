...
export class NotificationListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('notificationPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('notificationPage'));
    }
  ...
  create() {
    this.router.navigate(['notification-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['notification-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('notification/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('notification/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
