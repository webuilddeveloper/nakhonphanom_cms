...
@Component({
  selector: 'app-member-mobile-policy-application-criteria',
  templateUrl: './member-mobile-policy-application-criteria.component.html',
  styleUrls: ['./member-mobile-policy-application-criteria.component.css']
})
export class MemberMobilePolicyApplicationCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'policyRegister' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('policy/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
      this.listCategory = [];
      this.listCategory.push({value: '', display: 'ทั้งหมด'});
