...
export class EventListComponent implements OnInit {
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('event/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('event/reorder', { code: model }).subscribe(data => {
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('eventcalendar/read', {'skip':0,'limit':999999,'permission':this.permissionStr}).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
      ...
      this.excelService.exportAsExcelFile(result, 'รายงานแวดวงอาสา');
    }, err => {
      this.toastr.error(err.message, 'แจ้งเตือนระบบ', { timeOut: 2000 });
    });
  }
