...
export class PoiEditComponent implements OnInit {
  ...
  title = 'เพิ่มข้อมูลร้านค้าสมาชิก';
...
editModel: any = { fileUrl: '' , file: [],sequence: 10, language: 'th' };
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('poiPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('poiPage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('poiPage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลสถานที่น่าสนใจ';
        this.readComment()
      }else{
  ...
  create() {
    ...
    this.serviceProviderService.post('poi/create', this.editModel).subscribe(data => {
      ...
      if (model.status === 'S') {
        ...
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
            ...
            this.serviceProviderService.post('poi/gallery/create', element).subscribe(data => { }, err => { });
          });
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post('poi/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.serviceProviderService.post('poi/update', this.editModel).subscribe(data => {
      this.serviceProviderService.post('poi/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
  ...
  readCategory(param) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('poi/category/read', { language: param, permission: this.permission }).subscribe(data => {
        let model: any = {};
        model = data;
        this.listCategory = [];
  ...
  readComment() {
      this.serviceProviderService.post('poi/comment/read', { code: this.code }).subscribe(data => {
        let model: any = {};
        model = data;
  ...
  galleryRead() {
    this.serviceProviderService.post('m/poi/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  manageComment(param) {
    ...
    this.spinner.show()
    this.serviceProviderService.post('poi/comment/update', param).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  verifyComment(param) {
    this.spinner.show()
    this.serviceProviderService.post('poi/comment/verify', param).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  exportAsXLSX(): void {
    this.serviceProviderService.post('poi/comment/read', { skip: 0, limit: 999999 }).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
  ...
  back() {
    this.router.navigate(['poi'], { skipLocationChange: true });
  }
