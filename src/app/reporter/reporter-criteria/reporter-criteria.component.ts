...
@Component({
  selector: 'app-reporter-criteria',
  templateUrl: './reporter-criteria.component.html',
  styleUrls: ['./reporter-criteria.component.css']
})
export class ReporterCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'reporter' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('reporter/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
