...
@Component({
  selector: 'app-fund-criteria',
  templateUrl: './fund-criteria.component.html',
  styleUrls: ['./fund-criteria.component.css']
})
export class FundCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('fundPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('fundPage'));
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'fund' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('fund/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
