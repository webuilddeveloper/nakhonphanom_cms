...
export class PrivilegeEditComponent implements OnInit {
  ...
  title = 'เพิ่มข้อมูลสิทธิประโยชน์';
...
editModel: any = { fileUrl: '' , file: [],sequence: 10, language: 'th' };
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('privilegePage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('privilegePage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('privilegePage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลสิทธิประโยชน์';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('privilege/create', this.editModel).subscribe(data => {
    ...
    if(this.editModel.file != undefined){
      if (this.editModel.file.length > 0)
          ...
          this.serviceProviderService.post('privilege/gallery/create', element).subscribe(data => { }, err => { });
        });
      }
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post('privilege/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.serviceProviderService.post('privilege/update', this.editModel).subscribe(data => {

      this.serviceProviderService.post('privilege/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
  ...
  readCategory(param) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('privilege/category/read', { language: param, permission: this.permission }).subscribe(data => {
  ...
  galleryRead() {
    this.serviceProviderService.post('m/privilege/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['privilege'], { skipLocationChange: true });
  }
