...
@Component({
  selector: 'app-fund-category-edit',
  templateUrl: './fund-category-edit.component.html',
  styleUrls: ['./fund-category-edit.component.css']
})
export class FundCategoryEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'แก้ไขข้อมูลหมวดหมู่ข่าวสารประชาสัมพันธ์';
  category: any;
  ...
  create() {
    ...
    this.serviceProviderService.post('fund/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('fund/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('fund/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['fund-category'], { skipLocationChange: true });
  }
