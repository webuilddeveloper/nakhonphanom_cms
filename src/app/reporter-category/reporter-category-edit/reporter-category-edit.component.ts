...
@Component({
  selector: 'app-reporter-category-edit',
  templateUrl: './reporter-category-edit.component.html',
  styleUrls: ['./reporter-category-edit.component.css']
})
export class ReporterCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่ข้อเสนอแนะ';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่ข้อเสนอแนะ';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('reporter/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('reporter/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('reporter/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['reporter-category'], { skipLocationChange: true });
  }
