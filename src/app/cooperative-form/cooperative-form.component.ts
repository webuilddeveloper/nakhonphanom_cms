...
@Component({
  ...
  styleUrls: ['./cooperative-form.component.css']
})
export class CooperativeFormComponent implements OnInit {
  model: any = [];
  listModel: string = '';
  ...
  read() {
    ...
    this.serviceProviderService.post('cooperativeForm/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
