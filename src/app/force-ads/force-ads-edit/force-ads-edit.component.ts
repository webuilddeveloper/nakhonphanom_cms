...
@Component({
  selector: 'app-force-ads-edit',
  templateUrl: './force-ads-edit.component.html',
  styleUrls: ['./force-ads-edit.component.css']
})
export class ForceAdsEditComponent implements OnInit {
  listCategory: any = [];
  editModel: any = {};
  code: any;
  title = 'เพิ่มข้อมูลป้ายโฆษณาบังคับปิดเมนูย่อย';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลป้ายโฆษณาบังคับปิดเมนูย่อย';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('forceads/create', this.editModel).subscribe(data => {
      ...
      if (this.editModel.gallery.length > 0) {
        this.editModel.gallery.forEach(element => {
          ...
          this.serviceProviderService.post('forceads/gallery/create', element).subscribe(data => { }, err => { });
        });
      }
  ...
  read() {
    this.serviceProviderService.post('forceads/read', { code: this.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.serviceProviderService.post('forceads/update', this.editModel).subscribe(data => {

      this.serviceProviderService.post('forceads/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
  ...
  galleryRead() {
    this.serviceProviderService.post('m/forceads/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['force-ads'], { skipLocationChange: true });
  }
