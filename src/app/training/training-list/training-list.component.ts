...
export class TrainingListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('trainingPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('trainingPage'));
    }
  ...
  create() {
    this.router.navigate(['training-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['training-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('training/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('training/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
