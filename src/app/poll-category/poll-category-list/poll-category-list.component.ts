...
@Component({
  selector: 'app-poll-category-list',
  templateUrl: './poll-category-list.component.html',
  styleUrls: ['./poll-category-list.component.css']
})
export class PollCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('pollCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('pollCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['poll-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['poll-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('poll/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('poll/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
