...
@Component({
  selector: 'app-reporter-edit',
  templateUrl: './reporter-edit.component.html',
  styleUrls: ['./reporter-edit.component.css']
})
export class ReporterEditComponent implements OnInit {
  listCategory: any = [];
editModel: any = { fileUrl: '' , file: [],sequence: 10, language: 'th' };
  imageFile: string = '';
  code: any;
  title = 'เพิ่มข้อมูลข้อเสนอแนะ';
  ...
  isEditOrganization: boolean = false; // <----- Organization
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('reporterPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('reporterPage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('reporterPage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลสอบถาม / ข้อเสนอแนะ';
  ...
  create() {
    ...
    this.spinner.show();
    this.editModel.latLng = this.editModel.lat + ',' + this.editModel.lng
  ...
  async read() {
    this.spinner.show();
    this.serviceProviderService.post('reporter/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory(param) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('reporter/category/read', { language: param, permission: this.permission }).subscribe(data => {
        let model: any = {};
        model = data;
        this.listCategory = [];
  ...
  galleryRead() {
    this.serviceProviderService.post('m/reporter/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readMemberRegister() {
    debugger
    this.spinner.show();
    this.serviceProviderService.post('notification/member/read', { firstName: this.editModel.firstName, lastName:this.editModel.lastName }).subscribe(data => {
      let model: any = {};
      model = data;

      if (model.status == 'S') {
        this.notificationModel.items = model.objectData;
        this.notificationModel.token = model.objectData[0].token;
        this.notificationModel.isOnline = model.objectData[0].isOnline;
        this.notificationModel.title = this.editModel.title;
        this.notificationModel.category = 'reporterPage';

        this.pushNotification();
      } else {
        this.spinner.hide();
        this.toastr.error(model.message, 'แจ้งเตือนระบบ', { timeOut: 1000 });
      }
  ...
  pushNotification() {
    let sendDataPush = {};
    if(this.notificationModel.token != '' && this.notificationModel.token != null && this.notificationModel.token != undefined) {
      sendDataPush = {
        title: this.notificationModel.title,
        token: this.notificationModel.token,
        body: this.notificationModel.description,
        data: {
      ...
      this.spinner.hide();
     return this.toastr.warning('token not found.', 'แจ้งเตือนระบบ', { timeOut: 2000 });
    }
    ...
    this.serviceProviderService.post('notification/push', sendDataPush).subscribe(data => {
      ...
      this.serviceProviderService.post('notification/push', sendDataPush).subscribe(data => { this.spinner.hide(); }, err => {this.spinner.hide(); this.toastr.error(err.message, 'แจ้งเตือนระบบ', { timeOut: 2000 }); });

    });
  }

  createNotification() {
    this.serviceProviderService.post('notification/create', this.notificationModel).subscribe(async data => {
      let model: any = {};
      model = data;
      let letPush = false;
      if (model.status === 'S') {
        ...
        this.spinner.hide();
        this.toastr.success('ส่งแจ้งเตือนสำเร็จ', 'แจ้งเตือนระบบ', { timeOut: 1000 });
        setTimeout(() => {
          this.back();
        }, 1000);
  ...
  back() {
    this.router.navigate(['reporter'], { skipLocationChange: true });
  }
