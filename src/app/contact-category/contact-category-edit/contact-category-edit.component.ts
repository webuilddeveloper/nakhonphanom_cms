...
@Component({
  ...
  templateUrl: './contact-category-edit.component.html',
  styleUrls: ['./contact-category-edit.component.css']
})
export class ContactCategoryEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'เพิ่มข้อมูลหมวดหมู่เบอร์โทรฉุกเฉิน';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลหมวดหมู่เบอร์โทรฉุกเฉิน';
        this.read();
      }
    });
  ...
  create() {
    ...
    this.serviceProviderService.post('contact/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('contact/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('contact/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['contact-category'], { skipLocationChange: true });
  }
