...
export class WarningListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('warningPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('warningPage'));
    }
  ...
  create() {
    this.router.navigate(['warning-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['warning-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('warning/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('warning/reorder', { code: model }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('warning/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          },2000)
