...
@Component({
  selector: 'app-main-popup-edit',
  templateUrl: './main-popup-edit.component.html',
  styleUrls: ['./main-popup-edit.component.css']
})
export class MainPopupEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'เพิ่มข้อมูลป้ายโฆษณาบังคับปิดหน้าหลัก';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลป้ายโฆษณาบังคับปิดหน้าหลัก';
        this.read();
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('mainPopup/create', this.editModel).subscribe(data => {
      ...
      if (this.editModel.gallery.length > 0) {
        this.editModel.gallery.forEach(element => {
          ...
          this.serviceProviderService.post('mainPopup/gallery/create', element).subscribe(data => { }, err => { });
        });
      }
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post('mainPopup/read', { code: this.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.serviceProviderService.post('mainPopup/update', this.editModel).subscribe(data => {

      this.serviceProviderService.post('mainPopup/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
  ...
  galleryRead() {
    this.serviceProviderService.post('m/mainPopup/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['main-popup'], { skipLocationChange: true });
  }
