...
export class WarningEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('warningPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('warningPage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('warningPage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลอาสาเตือนภัย';
        this.read();
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('warning/create', this.editModel).subscribe(data => {
      ...
      if (this.editModel.gallery.length > 0) {
        this.editModel.gallery.forEach(element => {
          ...
          this.serviceProviderService.post('warning/gallery/create', element).subscribe(data => {
          }, err => { });
        });
      }
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post('warning/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.spinner.show();
    this.serviceProviderService.post('warning/update', this.editModel).subscribe(data => {

      this.serviceProviderService.post('warning/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
            // element.code = this.editModel.code; //เพิ่ม set active false ทั้วหมด
            element.reference = this.editModel.code;
            element.imageUrl = element.imageUrl;
            this.serviceProviderService.post('warning/gallery/create', element).subscribe(data => { }, err => { });
          });
        }
      }, err => { });
  ...
  readCategory(param: string) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('warning/category/read', { language: param, permission: this.permission }).subscribe(data => {
        let model: any = {};
        model = data;
        this.listCategory = [];
  ...
  galleryRead() {
    this.serviceProviderService.post('m/warning/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['warning'], { skipLocationChange: true });
  }
}
