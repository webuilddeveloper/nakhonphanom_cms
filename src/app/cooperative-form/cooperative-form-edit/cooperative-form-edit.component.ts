...
@Component({
  selector: 'app-cooperative-form-edit',
  templateUrl: './cooperative-form-edit.component.html',
  styleUrls: ['./cooperative-form-edit.component.css']
})
export class CooperativeFormEditComponent implements OnInit {
  listCategory: any = [];
  editModel: any = { fileUrl: '' , file: [] };
  code: any;
  title = 'เพิ่มข้อมูลคลังความรู้';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('cooperativeFormPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('cooperativeFormPage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('cooperativeFormPage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลคลังความรู้';
        this.read();
      }
  ...
  create() {
    ...
    // <----- Organization
    debugger
    this.serviceProviderService.post('cooperativeForm/create', this.editModel).subscribe(data => {
  ...
  read() {
    ...
    this.serviceProviderService.post('cooperativeForm/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.spinner.show();
    this.serviceProviderService.post('cooperativeForm/update', this.editModel).subscribe(data => {
  ...
  readCategory(param) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('m/cooperativeForm/category/read', { language: param, permission: this.permission }).subscribe(data => {
        let model: any = {};
        model = data;
        this.listCategory = [];
  ...
  checkSave() {
    ...
    if (this.code != '') {
      ...
      if (this.editModel.file != undefined) {
        if (this.editModel.file.length == 0) {
          this.toastr.warning('กรุณาเพิ่มไฟล์หนังสือ', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          isValid = true;
        }
      }
  ...
  back() {
    this.router.navigate(['cooperativeForm'], { skipLocationChange: true });
  }
}
