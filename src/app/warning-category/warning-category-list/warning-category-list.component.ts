...
@Component({
  selector: 'app-warning-category-list',
  templateUrl: './warning-category-list.component.html',
  styleUrls: ['./warning-category-list.component.css']
})
export class WarningCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('warningCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('warningCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['warning-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['warning-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('warning/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('warning/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
