...
editModel: any = { fileUrl: '' , file: [],sequence: 10, language: 'th' };
  ...
  imageFile: string = '';
  code: any;
  title = 'เพิ่มข้อมูลปฎิทินกิจกรรม';
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลปฎิทินกิจกรรม';
        this.read();
        this.readComment()
      }else{
  ...
  readComment() {
      this.serviceProviderService.post('eventCalendar/comment/read', { code: this.code }).subscribe(data => {
        let model: any = {};
        model = data;
  ...
  verifyComment(param) {
    this.spinner.show()
    this.serviceProviderService.post('eventCalendar/comment/verify', param).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  exportAsXLSX(): void {
    this.serviceProviderService.post('eventCalendar/comment/read', { skip: 0, limit: 999999 }).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
