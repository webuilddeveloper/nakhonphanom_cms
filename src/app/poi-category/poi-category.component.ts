...
@Component({
  selector: 'app-poi-category',
  templateUrl: './poi-category.component.html',
  styleUrls: ['./poi-category.component.css']
})
export class PoiCategoryComponent implements OnInit {
  model: any = [];
  listModel: string = '';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('poiCategoryPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('poiCategoryPage'));
