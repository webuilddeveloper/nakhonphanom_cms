...
@Component({
  selector: 'app-policy-marketing-list',
  templateUrl: './policy-marketing-list.component.html',
  styleUrls: ['./policy-marketing-list.component.css']
})
export class PolicyMarketingListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('policyMarketingPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('policyMarketingPage'));
    }
  ...
  create() {
    this.router.navigate(['policy-marketing-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['policy-marketing-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('policy/delete', { code: param.code, }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('policy/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
