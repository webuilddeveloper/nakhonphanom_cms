...
export class FundListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('fundPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('fundPage'));
    }
  ...
  create() {
    this.router.navigate(['fund-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['fund-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('fund/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('fund/reorder', { code: model }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('fund/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          },2000)
