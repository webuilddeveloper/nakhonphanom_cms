...
export class ContactEditComponent implements OnInit {
  ...
  code: any;
  title = 'เพิ่มข้อมูลเบอร์โทรฉุกเฉิน';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('contactPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('contactPage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('contactPage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลเบอร์โทรฉุกเฉิน';
        this.read();
      }
  ...
  create() {
    ...
    this.spinner.show();
    this.serviceProviderService.post('contact/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post('contact/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.spinner.show();
    this.serviceProviderService.post('contact/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  readCategory(param) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('contact/category/read', { language: param, permission: this.permission }).subscribe(data => {
        let model: any = {};
        model = data;
        this.listCategory = [];
