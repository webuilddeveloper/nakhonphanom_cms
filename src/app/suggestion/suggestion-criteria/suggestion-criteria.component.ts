...
@Component({
  selector: 'app-suggestion-criteria',
  templateUrl: './suggestion-criteria.component.html',
  styleUrls: ['./suggestion-criteria.component.css']
})
export class SuggestionCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'suggestion' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('suggestion/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
