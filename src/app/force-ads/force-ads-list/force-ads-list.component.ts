...
@Component({
  selector: 'app-force-ads-list',
  templateUrl: './force-ads-list.component.html',
  styleUrls: ['./force-ads-list.component.css']
})
export class ForceAdsListComponent implements OnInit {
  ...
  create() {
    this.router.navigate(['force-ads-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['force-ads-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('forceads/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('forceads/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
