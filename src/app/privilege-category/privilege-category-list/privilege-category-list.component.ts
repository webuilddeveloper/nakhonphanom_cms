...
@Component({
  selector: 'app-privilege-category-list',
  templateUrl: './privilege-category-list.component.html',
  styleUrls: ['./privilege-category-list.component.css']
})
export class PrivilegeCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('privilegeCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('privilegeCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['privilege-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['privilege-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('privilege/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('privilege/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
