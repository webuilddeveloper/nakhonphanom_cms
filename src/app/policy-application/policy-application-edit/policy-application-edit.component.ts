...
@Component({
  selector: 'app-policy-application-edit',
  templateUrl: './policy-application-edit.component.html',
  styleUrls: ['./policy-application-edit.component.css']
})
export class PolicyApplicationEditComponent implements OnInit {
  Editor = ClassicEditor;
  listCategory: any = [];
  editModel: any = {};
  code: any;
  title = 'เพิ่มข้อมูลนโยบายแอพพลิเคชัน';
  ...
  ngOnInit(): void {
    ...
    this.editModel.category = 'application';
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลนโยบายแอพพลิเคชัน';
        this.read();
      }
    });
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post(this.url.read, { code: this.code, category: 'application' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['policy-application'], { skipLocationChange: true });
  }
}
