...
@Component({
  selector: 'app-privilege-category-edit',
  templateUrl: './privilege-category-edit.component.html',
  styleUrls: ['./privilege-category-edit.component.css']
})
export class PrivilegeCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่สิทธิประโชยน์';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่สิทธิประโชยน์';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('privilege/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('privilege/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('privilege/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['privilege-category'], { skipLocationChange: true });
  }
