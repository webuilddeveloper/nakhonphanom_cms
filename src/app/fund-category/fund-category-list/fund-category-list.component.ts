...
@Component({
  selector: 'app-fund-category-list',
  templateUrl: './fund-category-list.component.html',
  styleUrls: ['./fund-category-list.component.css']
})
export class FundCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('fundCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('fundCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['fund-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['fund-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('fund/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('fund/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
