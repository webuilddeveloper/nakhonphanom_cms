...
@Component({
  selector: 'app-event-category-criteria',
  templateUrl: './event-category-criteria.component.html',
  styleUrls: ['./event-category-criteria.component.css']
})
export class EventCategoryCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('eventCategoryPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('eventCategoryPage'));
