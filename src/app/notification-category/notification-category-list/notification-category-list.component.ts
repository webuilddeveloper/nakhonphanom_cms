...
@Component({
  selector: 'app-notification-category-list',
  templateUrl: './notification-category-list.component.html',
  styleUrls: ['./notification-category-list.component.css']
})
export class NotificationCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('notificationCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('notificationCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['notification-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['notification-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('notification/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('notification/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
