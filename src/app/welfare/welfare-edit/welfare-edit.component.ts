...
export class WelfareEditComponent implements OnInit {
  ...
  code: any;
  title = 'เพิ่มข้อมูลสวัสดิการอาสา';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('welfarePage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('welfarePage'));
      ...
      this.permissionList = JSON.parse(localStorage.getItem('welfarePage'));
    }
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลสวัสดิการอาสา';
        this.read();
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('welfare/create', this.editModel).subscribe(data => {
      ...
      if (this.editModel.gallery.length > 0) {
        this.editModel.gallery.forEach(element => {
          ...
          this.serviceProviderService.post('welfare/gallery/create', element).subscribe(data => { }, err => { });
        });
      }
  ...
  read() {
    this.spinner.show();
    this.serviceProviderService.post('welfare/read', { code: this.code, permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.serviceProviderService.post('welfare/update', this.editModel).subscribe(data => {
      this.serviceProviderService.post('welfare/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
  ...
  readCategory(param: string) {
    ...
    if (this.editModel.language != '') {
      this.serviceProviderService.post('welfare/category/read', { language: param, permission: this.permission }).subscribe(data => {
        let model: any = {};
        model = data;
        this.listCategory = [];
  ...
  galleryRead() {
    this.serviceProviderService.post('m/welfare/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['welfare'], { skipLocationChange: true });
  }
