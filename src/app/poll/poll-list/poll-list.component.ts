...
export class PollListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('pollPage') != null) {
      let model: any = [];
      this.permission = JSON.parse(localStorage.getItem('pollPage'));
  ...
  create() {
    this.router.navigate(['poll-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['poll-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('poll/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('poll/reorder', { code: model }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('poll/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          },2000)
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('poll/read', {'skip':0,'limit':999999,'permission':this.permissionStr}).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
