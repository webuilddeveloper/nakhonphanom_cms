...
@Component({
  selector: 'app-welfare-category-list',
  templateUrl: './welfare-category-list.component.html',
  styleUrls: ['./welfare-category-list.component.css']
})
export class WelfareCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('welfareCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('welfareCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['welfare-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['welfare-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('welfare/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('welfare/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
