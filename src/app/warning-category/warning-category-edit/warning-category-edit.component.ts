...
@Component({
  selector: 'app-warning-category-edit',
  templateUrl: './warning-category-edit.component.html',
  styleUrls: ['./warning-category-edit.component.css']
})
export class WarningCategoryEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'เพิ่มข้อมูลหมวดหมู่อาสาเตือนภัย';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลหมวดหมู่อาสาเตือนภัย';
        this.read();
      }
    });
  ...
  create() {
    ...
    this.serviceProviderService.post('warning/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('warning/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('warning/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['warning-category'], { skipLocationChange: true });
  }
