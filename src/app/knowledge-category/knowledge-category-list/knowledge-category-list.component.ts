...
@Component({
  selector: 'app-knowledge-category-list',
  templateUrl: './knowledge-category-list.component.html',
  styleUrls: ['./knowledge-category-list.component.css']
})
export class KnowledgeCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('knowledgeCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('knowledgeCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['knowledge-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['knowledge-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('knowledge/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('knowledge/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
