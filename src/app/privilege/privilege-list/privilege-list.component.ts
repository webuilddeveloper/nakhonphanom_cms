...
export class PrivilegeListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('privilegePage') != null) {
      let model: any = [];
      this.permission = JSON.parse(localStorage.getItem('privilegePage'));
  ...
  create() {
    this.router.navigate(['privilege-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['privilege-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('privilege/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('privilege/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          },2000)
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('privilege/read', {'skip':0,'limit':999999,'permission':this.permissionStr}).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
      ...
      this.excelService.exportAsExcelFile(result, 'รายงานสิทธิประโยชน์');
    }, err => {
      this.toastr.error(err.message, 'แจ้งเตือนระบบ', { timeOut: 2000 });
    });
  }
