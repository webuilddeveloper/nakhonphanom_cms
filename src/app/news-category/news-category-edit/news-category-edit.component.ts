...
export class NewsCategoryEditComponent implements OnInit {
  ...
  editModel: any = {};
  code: any;
  title = 'แก้ไขข้อมูลหมวดหมู่ข่าวประชาสัมพันธ์';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลหมวดหมู่ข่าวประชาสัมพันธ์';
        this.read();
      }
    });
  ...
  create() {
    ...
    this.serviceProviderService.post('news/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('news/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('news/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['news-category'], { skipLocationChange: true });
  }
