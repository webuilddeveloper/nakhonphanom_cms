...
@Component({
  ...
  templateUrl: './training-category-list.component.html',
  styleUrls: ['./training-category-list.component.css']
})
export class TrainingCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('trainingCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('trainingCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['training-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['training-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('training/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('training/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
