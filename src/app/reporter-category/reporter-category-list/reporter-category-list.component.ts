...
@Component({
  selector: 'app-reporter-category-list',
  templateUrl: './reporter-category-list.component.html',
  styleUrls: ['./reporter-category-list.component.css']
})
export class ReporterCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('reporterCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('reporterCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['reporter-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['reporter-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('reporter/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('reporter/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
