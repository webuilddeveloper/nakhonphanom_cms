...
@Component({
  selector: 'app-notification-category-edit',
  templateUrl: './notification-category-edit.component.html',
  styleUrls: ['./notification-category-edit.component.css']
})
export class NotificationCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่แจ้งเตือน';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่แจ้งเตือน';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('notification/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('notification/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('notification/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['notification-category'], { skipLocationChange: true });
  }
