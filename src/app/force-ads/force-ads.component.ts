...
@Component({
  selector: 'app-force-ads',
  templateUrl: './force-ads.component.html',
  styleUrls: ['./force-ads.component.css']
})
export class ForceAdsComponent implements OnInit {
  model: any = [];
  listModel: string = '';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('forceAdsPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('forceAdsPage'));
  ...
  read() {
    ...
    this.serviceProviderService.post('forceads/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
