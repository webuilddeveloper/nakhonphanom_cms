...
@Component({
  selector: 'app-poi-category-edit',
  templateUrl: './poi-category-edit.component.html',
  styleUrls: ['./poi-category-edit.component.css']
})
export class PoiCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่สถานที่น่าสนใจ';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่สถานที่น่าสนใจ';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('poi/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('poi/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('poi/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['poi-category'], { skipLocationChange: true });
  }
