...
@Component({
  selector: 'app-knowledge-category-edit',
  templateUrl: './knowledge-category-edit.component.html',
  styleUrls: ['./knowledge-category-edit.component.css']
})
export class KnowledgeCategoryEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'เพิ่มข้อมูลหมวดหมู่คลังความรู้';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลหมวดหมู่คลังความรู้';
        this.read();
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('knowledge/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('knowledge/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('knowledge/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['knowledge-category'], { skipLocationChange: true });
  }
