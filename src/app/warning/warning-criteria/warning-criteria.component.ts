...
@Component({
  selector: 'app-warning-criteria',
  templateUrl: './warning-criteria.component.html',
  styleUrls: ['./warning-criteria.component.css']
})
export class WarningCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'warning' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('warning/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
