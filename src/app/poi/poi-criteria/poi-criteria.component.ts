...
@Component({
  selector: 'app-poi-criteria',
  templateUrl: './poi-criteria.component.html',
  styleUrls: ['./poi-criteria.component.css']
})
export class PoiCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'poi' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('poi/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
