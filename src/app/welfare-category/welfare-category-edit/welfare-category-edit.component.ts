...
@Component({
  selector: 'app-welfare-category-edit',
  templateUrl: './welfare-category-edit.component.html',
  styleUrls: ['./welfare-category-edit.component.css']
})
export class WelfareCategoryEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลหมวดหมู่สวัสดิการอาสา';
  editModel: any = {};
  code: any;
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลหมวดหมู่สวัสดิการอาสา';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('welfare/category/create', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('welfare/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('welfare/category/update', this.editModel).subscribe(data => {
      let model: any = {}
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['welfare-category'], { skipLocationChange: true });
  }
