...
@Component({
  selector: 'app-training-criteria',
  templateUrl: './training-criteria.component.html',
  styleUrls: ['./training-criteria.component.css']
})
export class TrainingCriteriaComponent implements OnInit {
  ...
  readSequence() {
    ...
    this.serviceProviderService.post('groupby/read', { permission: this.permission,title: 'training' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    ...
    this.serviceProviderService.post('training/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
